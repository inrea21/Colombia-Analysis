...
saveRDS(DM, "./DM.rds")
d<- readRDS("./DM.rds")
rgdal::writeOGR(d, "D:/uni/project MSc/Patia/mosaic", "DM", driver = "ESRI Shapefile")
...
simple.fit<- lm(DM_g_m2~dm_pred, data= testing)
summary(simple.fit)
plot(simple.fit)
...
ggplotRegression <- function(testing, Crude_Protein, cp_pred){
  ...
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point() + labs(x= 'Actual CP (%)', y= 'Predicted CP (%)')+
    stat_smooth(method = "lm", col = "red") + theme_bw()+
    labs(title = paste("A. (ii) \n Adj R² ",signif(summary(fit)$adj.r.squared, 5),
                       ";Intercept =",signif(fit$coef[[1]],5 ),
                       " ;Slope =",signif(fit$coef[[2]], 5),
...
ggplotRegression <- function(testing, In_vitro_digestibility, id_pred){
  ...
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point() + labs(x= 'Actual IVDMD (%)', y= 'Predicted IVDMD (%)')+
    stat_smooth(method = "lm", col = "red") + theme_bw()+
    labs(title = paste("A. (ii) \n Adj R²",signif(summary(fit)$adj.r.squared, 5),
                       ";Intercept =",signif(fit$coef[[1]],5 ),
                       " ;Slope =",signif(fit$coef[[2]], 5),
...
ggplotRegression <- function(testing, Ash, ash_pred){
  ...
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point() + labs(x= 'Actual Ash (%)', y= 'Predicted Ash (%)')+
    stat_smooth(method = "lm", col = "red") + theme_bw()+
    labs(title = paste("A. (ii) \n Adj R²",signif(summary(fit)$adj.r.squared, 5),
                       ";Intercept =",signif(fit$coef[[1]],5 ),
                       " ;Slope =",signif(fit$coef[[2]], 5),
