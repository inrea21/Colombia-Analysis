...
head(df)
df
df2 = cbind(df[19],df[27:36])
df3 = cbind(df[20],df[27:36])
df4 = cbind(df[21],df[27:36])
df5 = cbind(df[22],df[27:36])
...
ggplotRegression <- function(testing, DM_g_m2, dm_pred){
  
  require(ggplot2)
  
  fit <- lm(dm_pred~DM_g_m2, testing)
  
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point() + ylab("Predicted DM (g/m²)")+ xlab("Actual DM (g/m²)")+
    stat_smooth(method = "lm", col = "red") + theme_bw()+
    labs(title = paste("B. (ii) \n Adj R² = ",signif(summary(fit)$adj.r.squared, 5),
...
dm_pred1 <- predict(DM, training)
head(dm_pred1)
str(dm_pred1)

ggplotRegression <- function(training, DM_g_m2, dm_pred1){
  
  require(ggplot2)
  
  fit <- lm(dm_pred1~DM_g_m2, training)
  
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point() + ylab("Predicted DM (g/m²)")+ xlab("Actual DM (g/m²)")+
    stat_smooth(method = "lm", col = "red") + theme_bw()+
    labs(title = paste("B. (i) \n Adj R² = ",signif(summary(fit)$adj.r.squared, 5),
                       ";Intercept =",signif(fit$coef[[1]],5 ),
                       " ;Slope =",signif(fit$coef[[2]], 5),
                       " ;P =",signif(summary(fit)$coef[2,4], 5)))
}
...
ggplotRegression <- function(testing, Crude_Protein, cp_pred){
  
  require(ggplot2)
  
  fit <- lm(cp_pred~Crude_Protein, testing)
  
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point() + ylab("Predicted CP (%)")+ xlab("Actual CP (%)")+
    stat_smooth(method = "lm", col = "red") + theme_bw()+
    labs(title = paste("B. (ii) \n Adj R² = ",signif(summary(fit)$adj.r.squared, 5),
                       ";Intercept =",signif(fit$coef[[1]],5 ),
                       " ;Slope =",signif(fit$coef[[2]], 5),
...
cp_pred1 <- predict(CP, training)
head(cp_pred1)

ggplotRegression <- function(training, Crude_Protein, cp_pred1){
  
  require(ggplot2)
  
  fit <- lm(cp_pred1~Crude_Protein, training)
...
ggplotRegression <- function(testing, In_vitro_digestibility, id_pred){
  
  require(ggplot2)
  
  fit <- lm(id_pred~In_vitro_digestibility, testing)
  
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point() + ylab("Predicted IVDMD (%)")+ xlab("Actual IVDMD (%)")+
    stat_smooth(method = "lm", col = "red") + theme_bw()+
    labs(title = paste("B. (ii) \n Adj R² = ",signif(summary(fit)$adj.r.squared, 5),
                       ";Intercept =",signif(fit$coef[[1]],5 ),
                       " ;Slope =",signif(fit$coef[[2]], 5),
...
ip_pred1 <- predict(ID, training)
head(id_pred1)

ggplotRegression <- function(training, In_vitro_digestibility, id_pred1){
  
  require(ggplot2)
  
  fit <- lm(id_pred1~In_vitro_digestibility, training)
...
actuals_preds_id <- data.frame(cbind(actuals=testing$In_vitro_digestibility, predicteds=id_pred))  # make actuals_predicteds dataframe.
correlation_accuracy <- cor(actuals_preds_id) 
head(actuals_preds_id)
...
ggplotRegression <- function(testing, Ash, ash_pred){
  
  require(ggplot2)
  
  fit <- lm(ash_pred~Ash, testing)
  
  ggplot(fit$model, aes_string(x = names(fit$model)[2], y = names(fit$model)[1])) + 
    geom_point() + ylab("Predicted ASH (%)")+ xlab("Actual ASH (%)")+
    stat_smooth(method = "lm", col = "red") + theme_bw()+
    labs(title = paste("B. (ii) \n Adj R² = ",signif(summary(fit)$adj.r.squared, 5),
                       ";Intercept =",signif(fit$coef[[1]],5 ),
                       " ;Slope =",signif(fit$coef[[2]], 5),
...
ash_pred1 <- predict(ASH, training)
head(ash_pred1)

ggplotRegression <- function(training, Ash, ash_pred1){
  
  require(ggplot2)
  
  fit <- lm(ash_pred1~Ash, training)
